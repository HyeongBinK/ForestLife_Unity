        ...
        foreach (XmlElement node in root.ChildNodes)
        {
            ...
            foreach (var field in fields)
            {
                ...
                if (null != strValue) // 빈 문자열이 있을 수 깄기 때문에 null 확인만 한다.
                {
                    ...
                    if (System.Enum.TryParse<ITEM_TYPE>(strValue, out ITEM_TYPE IT)) value = IT;
                    else if (System.Enum.TryParse<HEALTYPE>(strValue, out HEALTYPE HT)) value = HT;
                    else if (int.TryParse(strValue, out int i)) value = i;
